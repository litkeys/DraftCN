...
describe('static-sources', () => {
  ...
  describe('viteConfigTemplate', () => {
    ...
    it('should import required modules', () => {
      expect(viteConfigTemplate).toContain("import { defineConfig } from 'vite'");
      expect(viteConfigTemplate).toContain("import react from '@vitejs/plugin-react'");
    ...
    it('should configure path alias for @', () => {
      ...
      expect(viteConfigTemplate).toContain('"@": path.resolve(__dirname, "./src")');
    });
  });

  describe('tsConfigTemplate', () => {
    it('should have correct compiler options', () => {
      expect(tsConfigTemplate.compilerOptions).toHaveProperty('target', 'ES2020');
      expect(tsConfigTemplate.compilerOptions).toHaveProperty('jsx', 'react-jsx');
      expect(tsConfigTemplate.compilerOptions).toHaveProperty('strict', true);
      expect(tsConfigTemplate.compilerOptions).toHaveProperty('skipLibCheck', true);
    });
    ...
    it('should reference tsconfig.node.json', () => {
      expect(tsConfigTemplate.references).toContainEqual({ path: './tsconfig.node.json' });
    });
  });

  describe('tsConfigNodeTemplate', () => {
    it('should have correct compiler options', () => {
      expect(tsConfigNodeTemplate.compilerOptions).toHaveProperty('composite', true);
      expect(tsConfigNodeTemplate.compilerOptions).toHaveProperty('skipLibCheck', true);
      expect(tsConfigNodeTemplate.compilerOptions).toHaveProperty('module', 'ESNext');
      expect(tsConfigNodeTemplate.compilerOptions).toHaveProperty('strict', true);
    });

    it('should include vite.config.ts', () => {
      expect(tsConfigNodeTemplate.include).toContain('vite.config.ts');
    });
  });

  describe('tailwindConfigTemplate', () => {
    it('should be a string', () => {
      expect(typeof tailwindConfigTemplate).toBe('string');
    });

    it('should configure content paths', () => {
      expect(tailwindConfigTemplate).toContain('./index.html');
      expect(tailwindConfigTemplate).toContain('./src/**/*.{js,ts,jsx,tsx}');
    });

    it('should extend theme with custom colors', () => {
      expect(tailwindConfigTemplate).toContain('colors: {');
      expect(tailwindConfigTemplate).toContain('border: "hsl(var(--border))"');
      expect(tailwindConfigTemplate).toContain('primary: {');
      expect(tailwindConfigTemplate).toContain('secondary: {');
    });

    it('should configure container', () => {
      expect(tailwindConfigTemplate).toContain('container: {');
      expect(tailwindConfigTemplate).toContain('center: true');
      expect(tailwindConfigTemplate).toContain('"2xl": "1400px"');
    });

    it('should include keyframes and animations', () => {
      expect(tailwindConfigTemplate).toContain('keyframes: {');
      expect(tailwindConfigTemplate).toContain('"accordion-down"');
      expect(tailwindConfigTemplate).toContain('"accordion-up"');
      expect(tailwindConfigTemplate).toContain('animation: {');
    });
  });
  ...
  describe('postcssConfigTemplate', () => {
    ...
    it('should configure tailwindcss and autoprefixer', () => {
      expect(postcssConfigTemplate).toContain('tailwindcss: {}');
      expect(postcssConfigTemplate).toContain('autoprefixer: {}');
    });
  });
  ...
  describe('eslintConfigTemplate', () => {
    ...
    it('should extend recommended configs', () => {
      ...
      expect(eslintConfigTemplate.extends).toContain('plugin:@typescript-eslint/recommended');
      expect(eslintConfigTemplate.extends).toContain('plugin:react-hooks/recommended');
    });
    ...
    it('should have custom rules', () => {
      expect(eslintConfigTemplate.rules).toHaveProperty('react-refresh/only-export-components');
      expect(eslintConfigTemplate.rules).toHaveProperty('@typescript-eslint/no-explicit-any', 'off');
    });
  });
  ...
  describe('indexHtmlTemplate', () => {
    ...
    it('should have required meta tags', () => {
      ...
      expect(indexHtmlTemplate).toContain('<meta name="viewport" content="width=device-width, initial-scale=1.0" />');
    });
    ...
    it('should include TypeScript entry point', () => {
      expect(indexHtmlTemplate).toContain('<script type="module" src="/src/index.tsx"></script>');
    });
  ...
  describe('indexTsxTemplate', () => {
    ...
    it('should import React and ReactDOM', () => {
      ...
      expect(indexTsxTemplate).toContain("import ReactDOM from 'react-dom/client'");
    });
  ...
  describe('readmeTemplate', () => {
    ...
    it('should document project structure', () => {
      ...
      expect(readmeTemplate).toContain('tailwind.config.js');
    });
  ...
  describe('utility functions', () => {
    ...
    describe('getTsConfigNodeString', () => {
      it('should return formatted JSON string', () => {
        const result = getTsConfigNodeString();
        expect(typeof result).toBe('string');

        const parsed = JSON.parse(result);
        expect(parsed).toEqual(tsConfigNodeTemplate);
  ...
  describe('template consistency', () => {
    ...
    it('should have consistent build tool references', () => {
      ...
      // TSConfig node includes vite.config.ts
      expect(tsConfigNodeTemplate.include).toContain('vite.config.ts');
    });
  });
});