...
describe('template-sources', () => {
  describe('templateSources', () => {
    it('should contain all required templates', () => {
      const requiredTemplates = ['hero1', 'navbar1', 'footer2', 'feature43', 'blog7', 'about3'];
    ...
    it('should preserve lucide-react imports', () => {
      ...
      const navbar1Source = templateSources['navbar1'];
      expect(navbar1Source).toContain("import { Book, Menu, Sunset, Trees, Zap } from 'lucide-react'");
    });
    ...
    it('should have proper export statements', () => {
      const templates = ['hero1', 'navbar1', 'footer2', 'feature43', 'blog7', 'about3'];
      templates.forEach(templateId => {
        const source = templateSources[templateId];
        const componentName = templateId.charAt(0).toUpperCase() + templateId.slice(1);
        expect(source).toContain(`export { ${componentName.replace(/\d+$/, (match) => match)} }`);
      });
    });
    ...
    it('should handle shadcnblocks imports correctly', () => {
      ...
      expect(footer2Source).toContain("import { Logo, LogoImage, LogoText } from './shadcnblocks/logo'");
  ...
  describe('getAvailableTemplates', () => {
    ...
    it('should include all expected templates', () => {
      const templates = getAvailableTemplates();
      const expectedTemplates = ['hero1', 'navbar1', 'footer2', 'feature43', 'blog7', 'about3'];
  ...
  describe('hasTemplateSource', () => {
    it('should return true for existing templates', () => {
      const existingTemplates = ['hero1', 'navbar1', 'footer2', 'feature43', 'blog7', 'about3'];
  ...
  describe('template content validation', () => {
    ...
    it('should contain default prop values', () => {
      ...
      expect(hero1Source).toContain("heading = 'Blocks Built With Shadcn & Tailwind'");
  ...
  describe('import path conversions', () => {
    it('should convert all UI component imports correctly', () => {
      const uiComponents = [
        'badge', 'button', 'accordion', 'navigation-menu',
        'sheet', 'card'
      ];
      ...
      Object.values(templateSources).forEach(source => {
        ...
        if (source.includes('Badge')) {
          const hasCorrectImport = source.includes("from './ui/badge'") || source.includes('from "./ui/badge"');
          expect(hasCorrectImport).toBe(true);
        }
        if (source.includes('Button')) {
          const hasCorrectImport = source.includes("from './ui/button'") || source.includes('from "./ui/button"');
          expect(hasCorrectImport).toBe(true);
        }
      });
    });
    ...
    it('should convert shadcnblocks component imports', () => {
      const footer2Source = templateSources['footer2'];
      expect(footer2Source).not.toContain('@/components/shadcnblocks/');
      expect(footer2Source).toContain('./shadcnblocks/logo');
    });
  });
});