  ...
  const handleCanvasMouseUp = useCallback(
    (e: React.MouseEvent) => {
      ...
      if (
        isActive &&
        draggedItem &&
        useAppStore.getState().sourceType === 'library'
      ) {
        const rect = e.currentTarget.getBoundingClientRect()
        ...
        if (template) {
          const newBlock = {
            id: `${draggedItem.typeId}-${Date.now()}-${Math.random()
              .toString(36)
              .substring(2, 11)}`,
...
type AppStore = AppState & AppActions & BlocksSlice & DragSlice & UISlice

// BlocksSlice - manages blocks and selection state
...
// UISlice - manages UI interactions and focus behavior
interface UISlice {
  // State
...
- **Sliced architecture** - Separate slices for blocks, drag, and UI interaction state
- **Selector patterns** - Dedicated selectors exported from each slice (e.g., `blocksSelectors`, `dragSelectors`)
...
- **Integrated selection** - Block selection managed within BlocksSlice using `selectedBlockIds` array and synchronized `selected` field on blocks
- **UI interactions** - Cross-component interactions managed via UISlice (search focus/blur)
...
```typescript
// Actual implementation in store/slices/blocks.ts
